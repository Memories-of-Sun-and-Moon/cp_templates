...
struct merge_sort_tree {
	...
	int count_leq(int l, int r, T q) {
		...
		while(l < r) {
			if(l & 1){
				ret += x[l].end() - lower_bound(x[l].begin(), x[l].end(), q);
				l++;
			}
			if(r & 1){
				r--;
				ret += x[r].end() - lower_bound(x[r].begin(), x[r].end(), q);
			}
			l >>= 1, r >>= 1;
		}
		return ret;
	...
	T sum_less(int l, int r, T q) {
		...
		while(l < r) {
			if(l & 1){
				ret += cs[l][lower_bound(x[l].begin(), x[l].end(), q) - x[l].begin()];
				l++;
			}
			if(r & 1){
				r--;
	...
	T sum_leq(int l, int r, T q) {
		...
		while(l < r) {
			if(l & 1){
				ret += cs[l].back() - cs[l][lower_bound(x[l].begin(), x[l].end(), q) - x[l].begin()];
				l++;
			}
			if(r & 1){
				r--;
				ret += cs[r].back() - cs[r][lower_bound(x[r].begin(), x[r].end(), q) - x[r].begin()];
			}
			l >>= 1, r >>= 1;
		}
		return ret;
